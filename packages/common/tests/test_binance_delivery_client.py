"""The COIN-M WebSocket API and the listen-key socket beside it."""

from __future__ import annotations

import asyncio
from decimal import Decimal

import pytest
from binance_delivery_stub import (
    API_KEY,
    LISTEN_KEY,
    FakeBinanceDeliveryApi,
    FakeBinanceDeliveryUser,
)
from mftik.exchange.binance.delivery import methods as m
from mftik.exchange.binance.delivery.client import BinanceDeliveryWsApi
from mftik.exchange.binance.delivery.protocol import BinanceWsError
from mftik.exchange.binance.delivery.user import BinanceDeliveryUserStream
from mftik.exchange.errors import ExchangeError


def _api(stub: FakeBinanceDeliveryApi, pem: str | None = None) -> BinanceDeliveryWsApi:
    return BinanceDeliveryWsApi(
        api_key=API_KEY if pem else None,
        api_secret=pem,
        url=stub.url,  # type: ignore[attr-defined]
        keepalive=0,
        retry_backoff=0.01,
    )


async def test_connecting_with_credentials_logs_the_session_on(
    delivery_api: FakeBinanceDeliveryApi, binance_key
) -> None:
    """The stub verifies the Ed25519 signature, so this covers signing too."""
    _key, pem = binance_key
    async with _api(delivery_api, pem) as api:
        assert api.logged_on
    assert delivery_api.logons == 1


async def test_a_public_client_never_logs_on(
    delivery_api: FakeBinanceDeliveryApi,
) -> None:
    async with _api(delivery_api) as api:
        assert not api.authenticated
        assert not api.logged_on
    assert delivery_api.logons == 0
    assert not delivery_api.calls(m.SESSION_LOGON)


async def test_a_reconnect_logs_on_again_before_anything_else(
    delivery_api: FakeBinanceDeliveryApi, binance_key
) -> None:
    """A lost socket is a lost authentication; the order path must not race it."""
    _key, pem = binance_key
    async with _api(delivery_api, pem) as api:
        delivery_api.drop_next = True
        await api.call(m.TIME)
        await asyncio.sleep(0.2)
        assert delivery_api.logons == 2
        assert api.logged_on


async def test_trading_without_credentials_is_refused_locally(
    delivery_api: FakeBinanceDeliveryApi,
) -> None:
    async with _api(delivery_api) as api:
        with pytest.raises(ExchangeError, match="trading call"):
            await api.place_order(symbol="BTCUSD_PERP", side="buy", quantity="1")


async def test_a_signed_call_still_stamps_itself_on_a_logged_on_session(
    delivery_api: FakeBinanceDeliveryApi, binance_key
) -> None:
    """The logon replaces the credential on each call, not the clock.

    The stub answers ``-1102`` for a signed method with no ``timestamp``, which
    is what Binance does — so this passing is the assertion.
    """
    _key, pem = binance_key
    async with _api(delivery_api, pem) as api:
        await api.fetch_balances()
    params = delivery_api.call(m.ACCOUNT_BALANCE)["params"]
    assert "timestamp" in params
    assert "apiKey" not in params, "the session says who we are"
    assert "signature" not in params


async def test_account_methods_are_unversioned(
    delivery_api: FakeBinanceDeliveryApi, binance_key
) -> None:
    """dapi answers ``account.balance`` / ``account.position``, not ``v2/``."""
    _key, pem = binance_key
    delivery_api.results[m.ACCOUNT_POSITION] = []
    async with _api(delivery_api, pem) as api:
        await api.fetch_balances()
        await api.fetch_positions()
    assert delivery_api.call(m.ACCOUNT_BALANCE)["method"] == "account.balance"
    assert delivery_api.call(m.ACCOUNT_POSITION)["method"] == "account.position"
    assert "v2/" not in delivery_api.call(m.ACCOUNT_BALANCE)["method"]
    assert "v2/" not in delivery_api.call(m.ACCOUNT_POSITION)["method"]


async def test_an_order_goes_out_in_binances_own_vocabulary(
    delivery_api: FakeBinanceDeliveryApi, binance_key
) -> None:
    _key, pem = binance_key
    delivery_api.results[m.ORDER_PLACE] = {
        "orderId": 1,
        "symbol": "BTCUSD_PERP",
        "status": "NEW",
        "clientOrderId": "c-1",
        "origQty": "1",
        "price": "40000",
    }
    async with _api(delivery_api, pem) as api:
        ack = await api.place_order(
            symbol="BTCUSD_PERP",
            side="buy",
            type="limit",
            quantity=Decimal("1.50"),
            price=Decimal("40000.10"),
            time_in_force="gtx",
            client_order_id="c-1",
            reduce_only=True,
        )
    params = delivery_api.call(m.ORDER_PLACE)["params"]
    assert params["symbol"] == "BTCUSD_PERP"
    assert params["side"] == "BUY"
    assert params["type"] == "LIMIT"
    assert params["timeInForce"] == "GTX"
    assert params["newClientOrderId"] == "c-1"
    assert params["reduceOnly"] is True
    assert params["quantity"] == "1.5"
    assert params["price"] == "40000.1"
    assert ack.order_id == 1


async def test_cancelling_needs_one_of_the_two_ids(
    delivery_api: FakeBinanceDeliveryApi, binance_key
) -> None:
    _key, pem = binance_key
    async with _api(delivery_api, pem) as api:
        with pytest.raises(ExchangeError, match="orderId or origClientOrderId"):
            await api.cancel_order("BTCUSD_PERP")


async def test_a_venue_error_comes_back_with_its_code(
    delivery_api: FakeBinanceDeliveryApi, binance_key
) -> None:
    _key, pem = binance_key
    delivery_api.errors[m.ORDER_PLACE] = {
        "code": -5022,
        "msg": "Due to the order could not be executed as maker, the Post Only "
        "order will be rejected.",
    }
    async with _api(delivery_api, pem) as api:
        with pytest.raises(BinanceWsError) as caught:
            await api.place_order(symbol="BTCUSD_PERP", side="buy", quantity="1")
    assert caught.value.code == -5022


async def test_positions_come_back_signed(
    delivery_api: FakeBinanceDeliveryApi, binance_key
) -> None:
    _key, pem = binance_key
    delivery_api.results[m.ACCOUNT_POSITION] = [
        {"symbol": "BTCUSD_PERP", "positionAmt": "-2", "entryPrice": "40000"}
    ]
    async with _api(delivery_api, pem) as api:
        rows = await api.fetch_positions()
    assert rows[0].position_amount == Decimal("-2")


# --- the listen-key socket -------------------------------------------------


def _user(
    stub: FakeBinanceDeliveryUser,
    *,
    keys: list[str] | None = None,
    pings: list[int] | None = None,
    keepalive_seconds: float = 60.0,
) -> BinanceDeliveryUserStream:
    issued = list(keys or [LISTEN_KEY, "listen-key-2"])

    async def start_key() -> str:
        return issued.pop(0)

    async def ping_key() -> None:
        if pings is not None:
            pings.append(1)

    return BinanceDeliveryUserStream(
        start_key=start_key,
        ping_key=ping_key,
        base_url=stub.url,  # type: ignore[attr-defined]
        keepalive_seconds=keepalive_seconds,
        keepalive=0,
        retry_backoff=0.01,
    )


async def test_a_listen_key_call_carries_neither_clock_nor_signature(
    delivery_api: FakeBinanceDeliveryApi, binance_key
) -> None:
    """``userDataStream.*`` is Binance's key-only class; a timestamp is ``-1101``."""
    _key, pem = binance_key
    async with _api(delivery_api, pem) as api:
        key = await api.start_user_stream()
    assert key == LISTEN_KEY
    assert delivery_api.call(m.USER_DATA_STREAM_START).get("params") is None


async def test_a_listen_key_call_off_a_cold_session_names_the_key(
    delivery_api: FakeBinanceDeliveryApi, binance_key
) -> None:
    _key, pem = binance_key
    api = _api(delivery_api, pem)
    await api.connect()
    try:
        api._logged_on = False
        await api.ping_user_stream()
    finally:
        await api.close()
    assert delivery_api.call(m.USER_DATA_STREAM_PING)["params"] == {"apiKey": API_KEY}


async def test_the_user_socket_is_opened_on_the_key_it_was_issued(
    delivery_user: FakeBinanceDeliveryUser,
) -> None:
    stream = _user(delivery_user)
    await stream.connect()
    try:
        await asyncio.sleep(0.05)
        assert delivery_user.listen_keys == [LISTEN_KEY]
        assert stream.listen_key == LISTEN_KEY
    finally:
        await stream.close()


async def test_account_events_route_to_their_own_views(
    delivery_user: FakeBinanceDeliveryUser,
) -> None:
    stream = _user(delivery_user)
    await stream.connect()
    try:
        orders = await stream.subscribe_order_updates()
        accounts = await stream.subscribe_account_updates()
        order_pump = asyncio.ensure_future(anext(orders))
        account_pump = asyncio.ensure_future(anext(accounts))
        await asyncio.sleep(0.05)

        await delivery_user.push(
            {
                "e": "ORDER_TRADE_UPDATE",
                "E": 1,
                "T": 1,
                "o": {
                    "s": "BTCUSD_PERP",
                    "S": "BUY",
                    "i": 7,
                    "X": "NEW",
                    "x": "NEW",
                },
            }
        )
        update = await asyncio.wait_for(order_pump, timeout=2.0)
        assert update.o.order_id == 7
        assert not account_pump.done()

        await delivery_user.push(
            {
                "e": "ACCOUNT_UPDATE",
                "E": 2,
                "T": 2,
                "a": {"m": "FUNDING_FEE", "B": [{"a": "BTC", "wb": "5", "cw": "5"}]},
            }
        )
        account = await asyncio.wait_for(account_pump, timeout=2.0)
        assert account.reason == "FUNDING_FEE"
    finally:
        await stream.close()


async def test_an_expired_listen_key_reopens_the_socket_with_a_new_one(
    delivery_user: FakeBinanceDeliveryUser,
) -> None:
    stream = _user(delivery_user, keys=[LISTEN_KEY, "listen-key-2"])
    await stream.connect()
    try:
        await asyncio.sleep(0.05)
        await delivery_user.push(
            {"e": "listenKeyExpired", "E": 1, "listenKey": LISTEN_KEY}
        )
        for _ in range(50):
            await asyncio.sleep(0.05)
            if delivery_user.connections > 1:
                break
        assert delivery_user.listen_keys == [LISTEN_KEY, "listen-key-2"]
        assert stream.listen_key == "listen-key-2"
    finally:
        await stream.close()


async def test_the_key_is_renewed_for_as_long_as_the_socket_is_up(
    delivery_user: FakeBinanceDeliveryUser,
) -> None:
    pings: list[int] = []
    stream = _user(delivery_user, pings=pings, keepalive_seconds=0.05)
    await stream.connect()
    try:
        await asyncio.sleep(0.2)
        assert pings, "the listen key was never renewed"
    finally:
        await stream.close()
    before = len(pings)
    await asyncio.sleep(0.15)
    assert len(pings) == before, "the renewal loop outlived the socket"
