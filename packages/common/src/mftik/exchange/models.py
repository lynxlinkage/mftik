...
:class:`~mftik.exchange.tickers.UniversalTicker`, carried as the one canonical
...
class InstrumentScoped(BaseModel):
    ...
    Identity is the :class:`~mftik.exchange.tickers.UniversalTicker`, carried as
    ...
    * ``BTCUSDT`` on Binance and ``BTCUSDT`` on Gate are different instruments
    ...
    :class:`~mftik.protocol.messages.SymbolInfo` states: one field cannot
    ...
    def symbol(self) -> str:
        ...
        canonical ticker are separated by :data:`~mftik.exchange.tickers.
