"""The connection machinery OKX's three sockets share.

OKX runs account pushes, market pushes and candles on separate connections,
but not on separate protocols: all three take ``{"id", "op", "args"}``,
answer on the id they were given (or on ``event``), push ``{"arg", "data"}``
frames that answer nothing, and expect the literal string ``ping``. So the
reconnect loop, the request/reply correlation, the heartbeat and the push
fan-out live here once.

**The heartbeat is not optional.** OKX closes a connection that has sent
nothing for ~30 seconds, whatever the WebSocket layer is doing. The ping is
the text ``ping`` and the pong is the text ``pong`` — neither is JSON —
which is why :meth:`OkxSocket._decode` special-cases them rather than
demanding an object.
"""
...
#: How often the application heartbeat goes out. OKX's idle timeout is 30s.
DEFAULT_PING_INTERVAL = 20.0


@dataclass
class _Pending:
    future: asyncio.Future[OkxResponse]
    op: str
...
class OkxSocket:
    ...
    async def __aenter__(self) -> OkxSocket:
        await self.connect()
        return self
    ...
    async def request(
        ...
        *,
        ...
        try:
            ...
            raise OkxWsError(None, f"no reply within {wait}s", op=op) from exc
        finally:
    ...
    async def _pump(self) -> None:
        ...
        async for raw in self._conn:
            ...
            if message is None:
                continue
            self._dispatch(OkxResponse(message))
    ...
    def _dispatch(self, resp: OkxResponse) -> None:
        if resp.is_pong:
            return
        if resp.is_reply:
...
__all__ = ["DEFAULT_PING_INTERVAL", "OkxSocket"]
