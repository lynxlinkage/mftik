...
Separate from :mod:`mftik.exchange.binance.spot.models` on purpose, though the
...
* **``@ticker`` carries no quote.** The futures 24h ticker has no ``b``/``a``
  fields at all, so a :class:`~mftik.exchange.models.Ticker` cannot be built from
...
* **A liquidation's ``S`` is the closing order's side, not the position's.**
  ...
  :class:`~mftik.exchange.models.Liquidation` states the *position's* side — so
...
#: :class:`~mftik.exchange.binance.future.private.BinanceFuturePrivateClient`.
BOTH = "BOTH"
...
class BinanceFutureAggTrade(BinanceMessage):
    ...
    can report, and a :class:`~mftik.exchange.models.Trade` built here carries
...
class BinanceFutureMarkPrice(BinanceMessage):
    ...
    :class:`~mftik.exchange.models.Ticker` would state something the venue did
...
class BinanceFutureTicker(BinanceMessage):
    ...
    :class:`~mftik.exchange.models.Ticker` whose bid, ask and last were the same
...
class BinanceFuturePositionRow(BinanceMessage):
    ...
    :class:`~mftik.exchange.oms.Position` states, so nothing has to combine a
...
class BinanceFutureAccountUpdate(BinanceMessage):
    ...
    def position_rows(self) -> list[BinanceFuturePositionRow]:
        ...
        Not converted here because a :class:`~mftik.exchange.oms.Position` needs
...
class BinanceListenKeyExpired(BinanceMessage):
    ...
    :class:`~mftik.exchange.binance.future.user.BinanceFutureUserStream`.
    """
...
class BinanceFutureMyTrade(BinanceMessage):
    ...
    **``realizedPnl`` is deliberately not carried onto the fill**, though the
    ...
    :class:`~mftik_db.models.history.CashFlowRow` refuses a session: the venue
