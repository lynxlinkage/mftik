...
:mod:`mftik.exchange.base`. The methods here resemble the spot connector's
...
* :class:`~mftik.exchange.binance.future.feed.BinanceFutureStream` — every live
  feed, across the two stream endpoints Binance splits them over.
* :class:`~mftik.exchange.binance.future.rest.BinanceFuturePublicRest` — the
...
from mftik.exchange.binance.future.feed import (
    DEFAULT_BOOK_LEVELS,
    DEFAULT_BOOK_SPEED,
    BinanceFutureStream,
)
...
#: open a pump that never yields. Refused here the way a missing
#: ``stream_*`` is refused at attach.
...
#: :mod:`mftik.exchange.intervals` refuses to accept from above, because it
...
class BinanceFuturePublicClient(BaseClient):
    ...
    async def _tickers(self, ticker: UniversalTicker) -> AsyncIterator[Ticker]:
        ...
        bid and no ask at all and a :class:`~mftik.exchange.models.Ticker`
