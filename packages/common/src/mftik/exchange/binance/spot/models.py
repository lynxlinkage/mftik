...
and keep its ``BTCUSDT`` spelling. Where a shared ``mftik.exchange`` model is a
...
#: (:attr:`~mftik.exchange.models.TimeInForce.POST_ONLY`), so the type it maps to
...
class BinanceAggTrade(BinanceMessage):
    ...
    on the resulting :class:`~mftik.exchange.models.Trade` is the aggregate id
...
class BinanceKlineEvent(BinanceMessage):
    ...
    def to_kline(self, ticker: UniversalTicker) -> Kline:
        ...
        :class:`~mftik.exchange.binance.spot.public.BinanceSpotPublicClient`, so
...
class BinanceBookTicker(BinanceMessage):
    ...
    arrival — see
    :meth:`~mftik.exchange.binance.spot.public.BinanceSpotPublicClient.stream_best_quote`.
    """
...
class BinanceSpotMyTrade(BinanceMessage):
    """One row of ``GET /api/v3/myTrades`` — an execution, after the fact.

    The same execution the user data stream reports as an
    :class:`BinanceExecutionReport` with ``x == "TRADE"``, and **``id`` here is
    that report's ``t``**. That identity is what lets a backfilled fill and a
    streamed one collapse onto a single record instead of double-counting the
    trade; :meth:`to_fill` and
    :meth:`BinanceExecutionReport.to_fill` must keep producing the same
    ``fill_id`` for it.

    Two fields read differently from their look-alikes on the public tape:

    * **``isBuyer`` is the account's own side**, not the maker flag. It is the
      answer to "did I buy", where the tape's ``m`` answers "was the buyer
      resting" — reading one as the other inverts the side.
    * **``isMaker`` is the account's role** in the match, and is carried
      through because the fee tier depends on it.

    There is deliberately no ``clientOrderId`` here: Binance does not put one on
    a trade row. An execution is tied back to the order that placed it through
    ``orderId``, which is what ``allOrders`` is read alongside this for.
    """
    ...
    time: int = 0
    is_buyer: bool = Field(default=False, alias="isBuyer")
    is_maker: bool = Field(default=False, alias="isMaker")

    @property
    def side(self) -> Side:
        return Side.BUY if self.is_buyer else Side.SELL

    def to_fill(self, ticker: UniversalTicker) -> Fill:
        """This execution as the shared model, ``client_order_id`` unset.

        Unset rather than guessed: the caller holds the ``orderId`` → order
        mapping this row cannot carry, and an empty value there is a fact about
        this endpoint, not about the fill.
        """
        return Fill(
            universal_ticker=str(ticker),
            fill_id=str(self.trade_id),
...
class BinanceSpotHistoricalOrder(BinanceMessage):
    """One row of ``GET /api/v3/allOrders`` — an order, in whatever state it ended.

    Unlike :class:`BinanceExecutionReport` this is a state, not an event: it is
    where the order stands now, with no record of how it got there. Which is
    all a history read wants, and it is the only place a terminal order can be
    recovered from once it has left the open-orders book.

    It answers for orders this platform never placed, too — the account's
    manual and third-party activity comes back on the same endpoint. Those have
    a ``clientOrderId`` Binance minted (``web_...``, or its own autogenerated
    form), which will not decode as one of ours; telling the two apart is the
    caller's job, and the id is passed through untouched so it can.
    """

    symbol: str = ""
    order_id: int = Field(default=0, alias="orderId")
    client_order_id_raw: str = Field(default="", alias="clientOrderId")
    price: Decimal = Decimal("0")
    ...
    @property
    def client_order_id(self) -> str | None:
        return self.client_order_id_raw or None
    ...
    @property
    def avg_price(self) -> Decimal | None:
        return avg(self.quote_total, self.executed_qty)

    def to_order(self, ticker: UniversalTicker) -> Order:
        """This order as the shared model, stamped at its last state change.

        ``updateTime`` rather than ``time``: the latter is when the order was
        placed, and for a history read the interesting moment is when it
        ...
        return Order(
            ...
            avg_price=self.avg_price,
            ts=secs(self.update_time or self.time),
        )
...
class BinanceBalanceUpdate(BinanceMessage):
    ...
    :class:`~mftik.exchange.models.Balance` states a position, and turning a
