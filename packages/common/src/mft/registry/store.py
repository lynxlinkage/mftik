...
from mft.registry.gate import StrategyClass, check_files
...
class RegistryStore:
    ...
    def list_remotes(self) -> list[Remote]:
        remotes = _load_remotes(self.remotes_path)
        return [Remote(name=n, url=u) for n, u in sorted(remotes.items())]

    def put_remote(self, name: str, url: str) -> Remote:
...
def _pick_class(classes: list[StrategyClass]) -> StrategyClass:
    if not classes:
        raise RegistryError(
            ...
            "mft_sts.strategy and subclass it"
        )
    if len(classes) > 1:
...
def _load_remotes(path: Path) -> dict[str, str]:
    if not path.is_file():
        return {}
    try:
...
def _dump_remotes(remotes: Mapping[str, str]) -> str:
    lines = ["[remotes]"]
    for name, url in sorted(remotes.items()):
        lines.append(f"{name} = {json.dumps(url)}")
    return "\n".join(lines) + "\n"
