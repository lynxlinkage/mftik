...
:class:`~mft.exchange.binance.future.user.BinanceFutureUserStream`'s job. That
...
:mod:`mft.exchange.binance.future.rest`.
"""
...
from mft.exchange.models import OrderBook, OrderType, Side, Ticker
...
class BinanceFutureWsApi(BinanceSocket):
    ...
    async def fetch_ticker(self, symbol: str, *, ticker: UniversalTicker) -> Ticker:
        """A :class:`~mft.exchange.models.Ticker`, which costs two calls here.
