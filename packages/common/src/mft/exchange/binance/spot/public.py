...
:mod:`mft.exchange.base`. The methods here resemble Gate's because the same
...
* :class:`~mft.exchange.binance.spot.feed.BinanceSpotStream` — every live feed.
  Binance pushes tape, book, candles, ticker and best quote from one host.
* :class:`~mft.exchange.binance.spot.client.BinanceSpotWsApi` — the on-demand
...
:mod:`mft.exchange.symbols`. Binance's spelling for spot happens to *be*
...
#: the one spelling :mod:`mft.exchange.intervals` refuses to accept from above,
