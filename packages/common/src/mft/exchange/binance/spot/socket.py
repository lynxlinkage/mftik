"""The connection machinery both Binance sockets share.

Binance splits what Gate serves on one URL across two: order entry, account
reads and market-data snapshots live on the **WebSocket API**
(``ws-api.binance.com``), while market pushes live on the **market streams**
socket (``stream.binance.com``). They are separate connections with separate
lifetimes and separate reasons to drop.

What they are *not* is separate protocols. Both take ``{"id", "method",
"params"}``, both answer on the ``id`` they were given, and both drop the
...
once, and the two clients differ only in what they say on a fresh socket
...
from mft.exchange.binance.spot.protocol import BinanceResponse, BinanceWsError
...
class BinanceSocket:
    ...
    async def connect(self) -> None:
        ...
        try:
            await self._open()
            # Before the pump starts, so a handshake reply can be read inline
            # off the same socket — the reconnect path reuses this for exactly
            # that reason.
            await self._on_open()
