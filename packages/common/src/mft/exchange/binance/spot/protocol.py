"""Binance spot WebSocket framing, Ed25519 signing, and response envelopes.

Both of Binance's sockets speak the same envelope — ``{"id", "method",
"params"}`` out, ``{"id", "status", "result" | "error"}`` back — which is why
one :class:`BinanceResponse` and one request/reply correlation path cover the
WebSocket API and the market streams alike. What differs is only *what* they
answer, not how.
...
def now_ms() -> int:
    ...
    :meth:`~mft.exchange.binance.spot.client.BinanceSpotWsApi.call`.
    """
