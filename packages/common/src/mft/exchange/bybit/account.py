...
:class:`~mft.exchange.bybit.models.BybitOrderAck`), so an adapter with order
...
would only filter out (:func:`~mft.exchange.bybit.channels.scoped`).

Authentication is one ``op: auth`` per connection, signed with HMAC-SHA256 over
a deadline (:func:`~mft.exchange.bybit.protocol.auth_frame`). It runs in
...
get two :class:`~mft.exchange.stream.EventStream` s fed by one subscription at
...
class BybitPrivateStream(BybitSocket):
    ...
    async def subscribe_executions(self) -> EventStream[BybitExecution]:
        ...
        :attr:`~mft.exchange.bybit.models.BybitExecution.is_fill`.
        """
