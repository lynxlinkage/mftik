...
spelling. Where a shared ``mft.exchange`` model is a faithful fit there is a
...
#: :data:`~mft.exchange.bybit.protocol._PRODUCT_BY_CATEGORY`, and the reason a
...
#: Bybit spells sides ``Buy``/``Sell``; :class:`~mft.exchange.models.Side` is
...
class BybitOrderUpdate(BybitMessage):
    ...
    def to_order(self, ticker: UniversalTicker) -> Order:
        ...
        ~mft.exchange.models.Order` states the instrument. The connector has
...
class BybitPosition(BybitMessage):
    ...
    def signed_size(self) -> Decimal:
        ...
        while :class:`~mft.exchange.oms.Position` states one signed quantity.
        """
...
class BybitTicker(BybitMessage):
    ...
    def quoted(self) -> bool:
        ...
        and turning that into a :class:`~mft.exchange.models.Ticker` would mean
...
class BybitOrderBook(BybitMessage):
    ...
    the two into whole books is :class:`~mft.exchange.bybit.feed.BybitBook`'s
    job.
...
class BybitKline(BybitMessage):
    ...
    def to_kline(self, ticker: UniversalTicker) -> Kline:
        ...
        :class:`~mft.exchange.bybit.public.BybitPublicClient`, so this model
...
def instrument_from_row(row: dict[str, Any]) -> Instrument:
    ...
    Both are read here so callers see one :class:`~mft.exchange.models.
