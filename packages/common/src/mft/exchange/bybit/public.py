...
:mod:`mft.exchange.base`. The methods here resemble Gate's and Binance's
...
* :class:`~mft.exchange.bybit.feed.BybitPublicStream` — the live feeds. **One
...
* :class:`~mft.exchange.bybit.rest.BybitPublicRest` — the on-demand reads. A
...
:class:`~mft.exchange.tickers.UniversalTicker` exists for, and the reason every
...
class BybitPublicClient(BaseClient):
    ...
    async def fetch_klines(
        ...
        :meth:`~mft.exchange.bybit.rest.BybitPublicRest.fetch_klines`.
        """
        self._ensure_connected()
        canonical = normalize_interval(interval)
    ...
    async def _tickers(self, ticker: UniversalTicker) -> AsyncIterator[Ticker]:
        ...
        :class:`~mft.exchange.models.Ticker` would mean inventing prices; it is
        dropped instead.
        """
        native, product = await self._resolve(ticker)
