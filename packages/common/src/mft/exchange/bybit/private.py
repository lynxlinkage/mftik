...
:mod:`mft.exchange.base`. It resembles Gate's and Binance's connectors because
...
* :class:`~mft.exchange.bybit.trade.BybitTradeSocket` — order entry. Fast, and
  it answers with two ids and no state.
* :class:`~mft.exchange.bybit.account.BybitPrivateStream` — what happened.
  Order updates and executions arrive here or nowhere.
* :class:`~mft.exchange.bybit.rest.BybitRest` — recon. "What is open" and
...
from mft.exchange.bybit.models import BybitOrderUpdate, category_of
...
class BybitPrivateClient(BaseClient):
    ...
    async def _fills(self) -> AsyncIterator[Fill]:
        ...
        Filtered on :attr:`~mft.exchange.bybit.models.BybitExecution.is_fill`:
    ...
    async def _balances(self) -> AsyncIterator[Balance]:
        """One :class:`~mft.exchange.models.Balance` per coin that moved.
