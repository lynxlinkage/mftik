"""GET /sts/strategies/{id}/yaml — rebuilding a past deploy as strategy.yml.

The submitted document is never stored, so the guarantee under test is that
what comes back parses to the spec that *was* stored. The repositories are
stubbed: none of this needs a database.
"""
