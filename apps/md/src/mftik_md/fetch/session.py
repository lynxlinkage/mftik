...
_KINDS: dict[str, _Kind] = {
    ...
    MD_FETCH_FUNDING_HISTORY: _Kind(
        model=MdFetchFundingHistory,
        read="fetch_funding_history",
        result_type=MD_FUNDING_HISTORY_RESULT,
        call=lambda read, req: read(_ticker(req), limit=req.limit),
        result=lambda req, ok, answer, reason, code: MdFundingHistoryResult(
            query_id=req.query_id,
            ticker=req.ticker,
            ok=ok,
            rates=list(answer or ()),
            reason=reason,
            error_code=code,
        ),
    ),
    MD_FETCH_OPEN_INTEREST: _Kind(
        model=MdFetchOpenInterest,
        read="fetch_open_interest",
        result_type=MD_OPEN_INTEREST_RESULT,
        call=lambda read, req: read(_ticker(req)),
        result=lambda req, ok, answer, reason, code: MdOpenInterestResult(
            query_id=req.query_id,
            ticker=req.ticker,
            ok=ok,
            open_interest=answer,
            reason=reason,
            error_code=code,
        ),
    ),
