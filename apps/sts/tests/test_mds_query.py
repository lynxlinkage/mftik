...
async def test_fetch_open_interest_reaches_its_own_hook(broker: Broker) -> None:
                ...
                await broker.publish(
                    ...
                    Envelope[MdOpenInterestResult].wrap(
                        MdOpenInterestResult(
                            ...
                            ticker=payload.ticker,
                            ok=False,
                            reason="venue 'Gate' does not serve fetch_open_interest",
    ...
    assert strategy.interest[0].ok is False
    assert (
        strategy.interest[0].error_code == QueryCode.MD_VENUE_UNSUPPORTED_READ
    )
    assert strategy.interest[0].open_interest is None
