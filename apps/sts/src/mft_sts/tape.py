...
MD records the trade feeds it pumps (see ``mft_md.tape``). This reads one back,
...
:class:`~mft_sts.ledger.StrategyLedger` reads ``td.ledger.{api_id}`` directly:
...
What comes back is the same :class:`~mft.exchange.models.Trade` /
:class:`~mft.exchange.models.AggTrade` the live hooks are handed, so a strategy
